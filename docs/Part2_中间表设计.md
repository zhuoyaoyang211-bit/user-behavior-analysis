...
从 `cleaned_data.parquet` 独立计算抽样样本的字段值，与中间表聚合结果逐一比对，验证计算逻辑正确性。抽检脚本：`src/verify_dims.py`。

### 9.2 抽样策略

| 表 | 抽样数 | 选样方式 |
|---|---|---|
| dim_user | 3个用户 | 2个 power_user（有收藏+加购+购买，验正常除法）+ 1个纯浏览用户（无收藏/加购/购买，验0÷0边界） |
| dim_item | 8个商品 | 5个热门（3+用户浏览，且收藏/加购/购买均有记录，验正常除法）+ 3个冷门（仅1个用户浏览且无收藏/加购/购买，验0÷0边界） |
| dim_category | 4个类目 | 2个热门（浏览/收藏/加购/购买都有，验正常除法）+ 2个冷门（仅浏览无收藏/加购/购买，验0÷N边界） |
| dim_time_daily | 3天 | 首日(11/18) + 中间日(12/03) + 末日(12/18) |
| 守恒校验 | 全量 | 四种行为量在 dim_user / dim_item / dim_category / dim_time_daily 四个维度加总 = 原始数据 |

**不抽检 dim_time_hourly / dim_time_weekday_hour 的原因**：三张时间表共用同一个构建函数 `_aggregate_time_metrics`，6个字段完全相同（全是计数，无除法），只是分组维度不同（按日 / 按(日,小时) / 按(星期,小时)）。验 dim_time_daily 即已覆盖该函数的全部代码逻辑，另外两张表无独立计算分支，守恒校验已间接覆盖其正确性。

### 9.3 抽检结果

| 校验类别 | 检查项数 | 通过 | 不一致 |
|---|---|---|---|
| dim_user（3个用户×16字段） | 48 | 48 | 0 |
| dim_item（8个商品×9字段） | 72 | 72 | 0 |
| dim_category（4个类目×10字段） | 40 | 40 | 0 |
| dim_time_daily（3天×6字段） | 18 | 18 | 0 |
| 全局守恒校验（4行为×4维度表） | 16 | 16 | 0 |
| **合计** | **194** | **194** | **0** |
...
| 行为 | 原始数据 | dim_user | dim_item | dim_category | dim_time_daily |
|---|---|---|---|---|---|
| 浏览(pv) | 11,550,581 | 11,550,581 | 11,550,581 | 11,550,581 | 11,550,581 |
| 收藏(fav) | 242,556 | 242,556 | 242,556 | 242,556 | 242,556 |
| 加购(cart) | 343,564 | 343,564 | 343,564 | 343,564 | 343,564 |
| 购买(buy) | 120,205 | 120,205 | 120,205 | 120,205 | 120,205 |

### 9.5 结论

194项抽检全部通过，中间表计算逻辑正确。覆盖了 power_user / 纯浏览用户、热门商品 / 冷门商品、热门类目 / 冷门类目、首末日期等边界情况，且四种行为量在四个维度表间完全守恒。
